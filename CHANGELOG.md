...
- **Memory (`memoryRelationMode: 'jev'`)** ⚠️ *experimental*: `kirograph_mem_compare`/`kirograph mem conflicts compare` previously computed nothing itself — the agent had to reason out the relation type (`supersedes`/`conflicts_with`/`compatible`/`scoped`/`related`/`not_conflict`) and confidence manually, then call `kirograph_mem_judge` to confirm its own answer, burning a full agent turn for a 6-way classification. With `relation` omitted, KiroGraph now classifies it via jev in one call and auto-judges it when confidence is at or above `memoryRelationConfidenceThreshold` (default `0.8`); below that, it's inserted `pending` exactly as before, for review via the existing judge flow. Default mode (`'agent'`) is unchanged — `--relation` stays required.
- **Wiki (`wikiContradictionMode: 'jev'`)** ⚠️ *experimental*: `wiki lint`'s contradiction check was a keyword heuristic (co-occurring negation words like `instead of`/`superseded` on FTS-similar pages). In jev mode, each FTS-similar candidate pair is judged by jev instead, reported only when confidence is at or above `wikiContradictionConfidenceThreshold` (default `0.7`). Candidate selection is unchanged; only the judgment is.
- **Security (`securityAuthDetectionMode: 'jev'`)** ⚠️ *experimental*: `attack-surface`'s `isAuthenticated` heuristic (substring match against a fixed name list) silently misclassifies a custom-named auth wrapper (e.g. `withSession`) as unauthenticated. In jev mode, only when the heuristic finds nothing does KiroGraph ask jev to confirm/override using the route + call-path names — the heuristic's positive matches are trusted as-is and never re-checked, keeping this cheap.
- New shared config: `jevApiKey` (falls back to `JEV_API_KEY` in `.kirograph/.env`, then in the real environment), `jevBaseUrl` (override for a local mock server or self-hosted deployment), `jevModel` (default `'jev-latest'`).
- **`.kirograph/.env`**: `loadConfig()` now loads `KEY=VALUE` pairs from `.kirograph/.env` into `process.env` on every command (a variable already set in the real environment is never overridden) — the way to give a project a real `JEV_API_KEY` without committing it to `config.json`. Minimal built-in parser, no new dependency.
- `src/jev/client.ts`: thin `JevClient` HTTP client for `POST /v1/systemone`, plus `createJevClientFromConfig()`.
- `test/jev/`: a local mock jev HTTP server (`mock-server.js`) and a `test.sh` covering all three integrations end-to-end. Auto-detects mode from `JEV_API_KEY`: unset → mock server, exact-value assertions (no real API key or network access needed, e.g. in CI); set → the real API, with the key routed through `.kirograph/.env` to exercise that loading path too, and exact-value assertions relaxed to warnings for whatever depends on the live model's own judgment (wiring-level assertions — valid relation type, confidence in range, DB persistence consistent with what was returned, clean error handling — stay hard failures in both modes).
- **Installer**: `kirograph install` now asks whether to enable jev-powered classification once at least one of the three modules it attaches to (Memory, Wiki, Security) is enabled — skipped entirely otherwise, since there's nothing for it to classify. Offered in `full`, `profiles`, and `custom` install modes (not `core`, which never turns on those modules). Available for `Full` and any `profiles` preset that enables Memory/Wiki/Security, not just `Custom`. Accepting it sets the relevant `*Mode: 'jev'` flags only for the modules actually enabled by that install. When accepted, the installer creates `.kirograph/.env` with a `JEV_API_KEY=` placeholder (never overwriting an existing one), appends it to the project's `.gitignore` if one exists, and prints a final setup step after installing: where the file is, what to set, and where to get a key. `ensureJevEnvFile()` in `src/bin/installer/common.ts`.
...
- **Security (Cargo, Gradle, NuGet, RubyGems, Composer, Pub, Hex)**: same class of gap — the manifest plugins for `Cargo.lock`, `gradle.lockfile`, `packages.lock.json`, `Gemfile.lock`, `composer.lock`, `pubspec.lock`, and `mix.lock` only used the lock file to resolve versions for packages already declared directly in the manifest, discarding every purely transitive package. Extended the same fix to all seven. Swift already used `Package.resolved` as its authoritative full list and needed no change; `requirements.txt` has no dependency-tree format and remains `incomplete` by design, matching Go's existing behavior.
- **Security (Maven)**: two compounding gaps meant `kirograph vulns` reported "No vulnerabilities found" for a real Spring Boot project with known CRITICAL CVEs (issue #39 follow-up). First, `<dependency>` entries with no explicit `<version>` — the common case for anything managed by a BOM parent like `spring-boot-starter-parent` — never got a resolved version and were silently filtered out before any OSV query happened; transitive-only packages (e.g. `tomcat-embed-core`, pulled in via `spring-boot-starter-web`) never appeared in `pom.xml` at all, so they were invisible to scanning regardless. Maven has no lock file, so `parseMavenManifest` now reads a pre-generated `mvn dependency:tree -DoutputFile=dependency-tree.txt` text file (next to `pom.xml`, or under `target/`) as the closest static equivalent — it fills in resolved versions for BOM-managed dependencies and adds transitive-only packages, mirroring how the npm plugin layers `package-lock.json` on top of `package.json`. Second, `<dependencyManagement>` entries (BOM imports, version overrides) were being matched by the same `<dependency>` regex used for real dependencies and miscounted as direct project dependencies — fixed by stripping that block before scanning. `kirograph vulns` now also hints at the `mvn dependency:tree` command when Maven dependencies are still unresolved.
- **Security (reachability persistence)**: `ReachabilityAnalyzer.analyze()` computed a verdict but only persisted it to `sec_reachability` on its main success path — the two early-exit cases (no dependency link found, or zero entry points anywhere in the project) returned a verdict in memory without ever writing it to the database. Since `kirograph vulns` reads the verdict via a `LEFT JOIN` on that table, affected vulnerabilities showed `[pending]` forever, no matter how many times `kirograph index` was rerun (issue #39 follow-up, surfaced once the Maven fix above made real Java vulnerabilities scannable for the first time).
- **Security (reachability false negative)**: a dependency that's never explicitly imported or called anywhere in the indexed code — e.g. an embedded servlet container like `tomcat-embed-core`, wired in via classpath scanning/reflection rather than an explicit reference — has zero incoming `calls`/`imports`/`references` edges in the graph. The reverse BFS from such a dependency was a guaranteed no-op (always "no path, no unresolved imports"), so it fell through to `not_affected` regardless of whether the code was actually analyzed (issue #39 follow-up). The call graph has no signal about the dependency at all in this case, which is exactly what `under_investigation` is for — `ReachabilityAnalyzer` now checks for zero incoming traversal edges before running the BFS and reports `under_investigation` immediately when that's the case. Dependencies that are referenced somewhere but genuinely unreachable, or genuinely reachable, are unaffected.
...
- **Security (npm duplicate versions)**: a package name can resolve to more than one distinct version in the same npm tree (e.g. a patched `qs@6.16.0` at the top level alongside a vulnerable `qs@6.15.3` nested under `body-parser`) — a Dependency_Node only records one `resolved_version`, so only that copy was ever checked against OSV, silently missing vulnerabilities that only affect the other installed copy. `VulnerabilityDatabaseClient` now runs a follow-up pass (`enrichDuplicateVersions`) that finds every other distinct version present in `package-lock.json` (new `findNpmDuplicateVersions` in the npm plugin) and checks each of them too, linking any additional findings to the same dependency — without changing what's stored as `resolved_version` or any DB schema. npm-only for now; scoped to the project-root `package-lock.json` (not per-package lock files in a monorepo, nor pnpm/yarn).
- **Extraction**: `kirograph sync`/`index` could hang or balloon in memory on large repos with many changed files. `getParser()` creates a fresh `web-tree-sitter` `Parser` per file, and `extractFile()`/`extractNotebook()` call `parser.parse()` to get a `Tree` — both are WASM-backed objects whose linear memory isn't reclaimed by JS garbage collection, only by their explicit `.delete()` lifecycle methods, which the extraction path never called. Every parsed file (and every parsed notebook) permanently leaked its `Parser` and `Tree`, so `sync` runs touching thousands of files accumulated unbounded WASM memory until the process stalled or was killed. Both `extractFile()` and `extractNotebook()` now wrap parsing in `try/finally` blocks that call `tree.delete()` and `parser.delete()` on every exit path, including early returns (unparseable YAML templates, WASM parse crashes) and thrown errors. Verified with a synthetic 300-iteration `extractFile()` harness: `external` memory (where WASM/native allocations are tracked) grew unboundedly before the fix (33.7 MiB → 57 MiB) and stayed flat after it (33.7 MiB constant).
