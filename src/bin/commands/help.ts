...
const GROUPS: Group[] = [
  ...
  },
  {
    ...
    commands: [
      ...
      { name: 'wiki lint',       desc: 'Health check the wiki for broken links, orphans, contradictions. wikiContradictionMode: "jev" replaces the keyword heuristic with an actual judgment call.' },
  ...
  },
  {
    ...
    commands: [
      ...
      { name: 'attack-surface',      args: '[path]', desc: 'Map attack surface: routes → vulnerable deps with hop count and auth status. securityAuthDetectionMode: "jev" catches custom-named auth wrappers the heuristic misses.', opts: ['--limit <n>', '--public-only', '--format json'] },
